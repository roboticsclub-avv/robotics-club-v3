# Phase S1 - Architecture Discovery Report (Revised)
...
| **Recruitment Wizard** | Typeform-style wizard split-layout steps in HTML/CSS | Traditional form layout | **Recreate V1 Wizard as Next.js React Components** | Preserves V1 step-by-step UX while wrapping it in Next.js structure. (See Join Us Decision) |
...
The V1 `dashboard.html` maps to the tabbed modular V2 dashboard framework (`src/app/dashboard/components`) as follows:

*   **V1 Applicants Module ➔ `ApplicantsTab.js`**
    *   *Functional Mapping*: Renders status cards (Total, Pending, Accepted), lists applicants, triggers approvals (updates role to `member`, generates `RC-XXXX` memberId, flags status as `accepted`) or rejections (status `rejected`), and handles record deletions.
*   **V1 Event Management Module ➔ `EventsTab.js`**
    *   *Functional Mapping*: Renders new event builder forms, binds fields (title, date, comingSoon, image, description, link), toggles disabled dates on Coming Soon checkbox changes, and displays active events list tables with Edit/Delete buttons.
*   **V1 Hardware Inventory Module ➔ `InventoryTab.js`**
    *   *Functional Mapping*: Renders hardware creation forms, updates component details (name, category, totalQuantity, image), and tracks active quantity inventories.
*   **V1 Hardware Allocation Module ➔ `AllocationsTab.js`**
    *   *Functional Mapping*: Powers hardware loan operations. Handles member verification scans (`memberId` lookup), filters items with positive availability levels, registers expected returns, lists issued rows, and handles hardware return loops.
*   **Excluded/Reserved Tabs**: V2's `MeetingsTab` and `SettingsTab` are out of scope for the V3 migration and will be hidden or deactivated.

### Join Us Decision

*   **Decision**: Recreate V1's Typeform-style split-layout steps recruitment wizard instead of V2's standard static form.
*   **UX Execution**: Build a multi-step slide wizard React component flow inside `/join-us/page.js`, preserving:
    *   V1's split layout (Image-side showcasing the program, Form-side displaying slides).
    *   Dynamic progress indicator bar and numerical tracker steps.
    *   Card layouts for multiple-choice steps (Year selection, Area of Interest selection).
    *   OK checks, keyboard event mappings (pressing Enter advances to the next step, Ctrl+Enter submits), and fade transitions.
*   **Rationale**: Meets the design core principle: "The final website should feel like Version 1 while being engineered like Version 2" and ensures compliance with visual sources metadata ("Recruitment -> V1").
...
V3 maps resource routes to ensure compatibility:
1.  **Media Assets**: Copy `current-v1/media/*` (student faces, icons, banners) to `public/media/` in the V3 layout.
2.  **Scene Codes**: Transfer `reference-v2/public/scene.splinecode` to V3 `public/` directory for local WebGL fetches.
3.  **Background Elements**: Migrate video files (`6761a89fa66e8645430254.mp4`) directly to the public folder.

### Route Protection Strategy

1.  **Middleware Gate**: Integrate Next.js middleware checking for active Firebase Session Cookies on `/dashboard` and `/member` pathways. Redirects unauthorized accesses directly to `/login`.
2.  **API Handler Guards**: API Routes extract request tokens and query `firebase-admin` authentication modules to match client claims. Refuses updates for standard `role != 'admin'` profile IDs.

### Environment Variable Strategy

Create a `.env.local` configuration for developmental boundaries:
*   **Client Settings**:
    *   `NEXT_PUBLIC_FIREBASE_API_KEY`: Browser authentication key.
    *   `NEXT_PUBLIC_FIREBASE_AUTH_DOMAIN`, `NEXT_PUBLIC_FIREBASE_PROJECT_ID`
    *   `NEXT_PUBLIC_FIREBASE_STORAGE_BUCKET`, `NEXT_PUBLIC_FIREBASE_MESSAGING_SENDER_ID`, `NEXT_PUBLIC_FIREBASE_APP_ID`
    *   `NEXT_PUBLIC_EMAILJS_PUBLIC_KEY`, `NEXT_PUBLIC_EMAILJS_SERVICE_ID`, `NEXT_PUBLIC_EMAILJS_ACCEPTANCE_TEMPLATE_ID`, `NEXT_PUBLIC_EMAILJS_REJECTION_TEMPLATE_ID`
*   **Server Settings**:
    *   `FIREBASE_ADMIN_PROJECT_ID`
    *   `FIREBASE_ADMIN_CLIENT_EMAIL`
    *   `FIREBASE_ADMIN_PRIVATE_KEY` (properly parses literal escape signs: `replaceAll('\\n', '\n')`)

### Branch Strategy

All Phase development operates inside isolated git boundaries:
*   **Staging Hub**: `migration-v3`
*   **Main Hub**: `main` (for final reviews)
*   **Phase Tranches**:
    *   `feature/p2-nextjs-setup`
    *   `feature/p3-loader`
    *   `feature/p4-hero-glitch`
    *   `feature/p4b-themes`
    *   `feature/p4c-alerts`
    *   `feature/p4d-errors`
    *   `feature/p5-gallery`
    *   `feature/p6-events`
    *   `feature/p7-performance`
    *   `feature/p8-seo`
*   **PR Rule**: Direct merges to target staging require validation tests checklist validation before approval.
